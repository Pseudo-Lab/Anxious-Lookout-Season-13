...
For a stronger API-level restore check, root can run `restore-check.sh /absolute/path/to/archive.tar.gz` after pre-pulling `rancher/k3s:v1.36.4-k3s1`. It extracts only the copied datastore/token/TLS/credential tree into a new root-owned temporary directory, starts an unprivileged Docker container with no network, published ports or agent, checks API readiness and retained policy/namespace records, and compares only hashes of the decrypted `k3s-serving` Secret data. Neither Secret data nor kubeconfig is printed. All temporary container/data resources are removed by the exit trap. If API startup fails, private logs remain at `/var/log/anxious-k3s-restore-failure.log`; inspect them locally and redact before any issue update. Certificate rotation can cause a hash mismatch, which the script reports as failure requiring investigation rather than claiming successful equality.
